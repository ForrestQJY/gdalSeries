 ...
 */
float *
gb::GDALDatasetReader::readRasterHeights(const GDALTiler &tiler, GDALDataset *dataset, const TileCoordinate &coord, gb::i_tile tileSizeX, gb::i_tile tileSizeY) {
  GDALTile *rasterTile = createRasterTile(tiler, dataset, coord); // the raster associated with this tile coordinate

  const gb::i_tile TILE_CELL_SIZE = tileSizeX * tileSizeY;
  ...
  if (heightsBand->RasterIO(GF_Read, 0, 0, tileSizeX, tileSizeY,
    ...
    throw GBException("Could not read heights from raster");
  }
  delete rasterTile;
...
GDALTile *
gb::GDALDatasetReader::createRasterTile(const GDALTiler &tiler, GDALDataset *dataset, const TileCoordinate &coord) {
  return tiler.createRasterTile(dataset, coord);
}

/// Create a VTR raster overview from a GDALDataset
GDALDataset *
gb::GDALDatasetReader::createOverview(const GDALTiler &tiler, GDALDataset *dataset, const TileCoordinate &coord, int overviewIndex) {
...
/// The destructor
gb::GDALDatasetReaderWithOverviews::~GDALDatasetReaderWithOverviews() {
  reset();
}

/// Read a region of raster heights into an array for the specified Dataset and Coordinate
float *
gb::GDALDatasetReaderWithOverviews::readRasterHeights(GDALDataset *dataset, const TileCoordinate &coord, gb::i_tile tileSizeX, gb::i_tile tileSizeY) {
  ...
  while (!rasterOk) {
    ...
    if (heightsBand->RasterIO(GF_Read, 0, 0, tileSizeX, tileSizeY,
      ...
      if (psOverview) {
        ...
        delete rasterTile;
        CPLFree(rasterHeights);
        throw GBException("Could not create an overview of current GDAL dataset");
      }
    }
    else {
      rasterOk = true;
...
void 
gb::GDALDatasetReaderWithOverviews::reset() {
  mOverviewIndex = 0;
