	...
	options(options)
{
	...
	if (poDataset != NULL) {
		...
		if (poDataset->GetGeoTransform(adfGeoTransform) == CE_None) {
			...
			throw GBException("Could not get transformation information from source dataset");
		}

		// Find out whether the dataset SRS matches that of the grid
		const char* srcWKT = poDataset->GetProjectionRef();
		if (!strlen(srcWKT))
			throw GBException("The source dataset does not have a spatial reference system assigned");
		...
		if (!srcSRS.IsSame(&gridSRS)) { // it doesn't match
			...
			case OGRERR_CORRUPT_DATA:
				throw GBException("The source spatial reference system appears to be corrupted");
				break;
			case OGRERR_UNSUPPORTED_SRS:
				throw GBException("The source spatial reference system is not supported");
				break;
			default:
				throw GBException("There is an unhandled return value from `srcSRS.Validate()`");
			}
			...
			double minY = std::min(std::min(y[0], y[1]), std::min(y[2], y[3]));
			double maxY = std::max(std::max(y[0], y[1]), std::max(y[2], y[3]));
			bounds_latLngAlt = CRSBounds(minX, minY, maxX, maxY); // set the bounds


			OGRCoordinateTransformation* transformer = OGRCreateCoordinateTransformation(&srcSRS, &gridSRS);
			if (transformer == NULL) {
				throw GBException("The source dataset to tile grid coordinate transformation could not be created");
			}
			else if (!transformer->Transform(4, x, y)) {
				delete transformer;
				throw GBException("Could not transform dataset bounds to tile spatial reference system");
			}
			delete transformer;

			// Get the min and max values of the transformed coordinates
			minX = std::min(std::min(x[0], x[1]), std::min(x[2], x[3]));
			...
			bounds_coordinates = CRSBounds(minX, minY, maxX, maxY); // set the bounds
			mResolution = bounds_coordinates.getWidth() / poDataset->GetRasterXSize(); // set the resolution

			// cache the SRS string for use in reprojections later
			char* srsWKT = NULL;
			if (gridSRS.exportToWkt(&srsWKT) != OGRERR_NONE) {
				CPLFree(srsWKT);
				throw GBException("Could not create grid WKT string");
			}
			crsWKT = srsWKT;
			CPLFree(srsWKT);
			srsWKT = NULL;
			...
			bounds_coordinates = bounds;         // use the existing dataset bounds
			bounds_latLngAlt = bounds;
...
GDALTiler::GDALTiler(const GDALTiler& other) :
	...
	bounds_coordinates(other.bounds_coordinates),
...
GDALTiler::createRasterTile(GDALDataset* dataset, const TileCoordinate& coord) const {
	...
	if (GDALSetGeoTransform(tile->dataset, adfGeoTransform) != CE_None) {
		throw GBException("Could not set geo transform on VRT");
	}
...
GDALTiler::createRasterTile(GDALDataset* dataset, double(&adfGeoTransform)[6]) const {
	if (dataset == NULL) {
		throw GBException("No GDAL dataset is set");
	}
	...
	if (!strlen(pszSrcWKT))
		throw GBException("The source dataset no longer has a spatial reference system assigned");
	...
	if (transformerArg == NULL) {
		...
		throw GBException("Could not create image to image transformer");
	}
	...
	if (hWrkSrcDS == NULL) {
		...
		if (transformerArg == NULL) {
			GDALDestroyWarpOptions(psWarpOptions);
			throw GBException("Could not create overview image to image transformer");
		}
	...
	if (options.errorThreshold) {
		...
		if (psWarpOptions->pTransformerArg == NULL) {
			GDALDestroyWarpOptions(psWarpOptions);
			GDALDestroyGenImgProjTransformer(transformerArg);
			throw GBException("Could not create linear approximator");
		}
	...
	if (hDstDS == NULL) {
		GDALDestroyGenImgProjTransformer(transformerArg);
		throw GBException("Could not create warped VRT");
	}
	...
	if (GDALSetProjection(hDstDS, pszGridWKT) != CE_None) {
		...
		throw GBException("Could not set projection on VRT");
	}
