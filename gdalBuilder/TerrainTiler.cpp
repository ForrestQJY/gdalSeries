...
void 
gb::TerrainTiler::prepareSettingsOfTile(TerrainTile *terrainTile, const TileCoordinate &coord, float *rasterHeights, gb::i_tile tileSizeX, gb::i_tile tileSizeY) const {
...
TerrainTile *
gb::TerrainTiler::createTile(GDALDataset *dataset, const TileCoordinate &coord) const {
  // Copy the raster data into an array
  float *rasterHeights = gb::GDALDatasetReader::readRasterHeights(*this, dataset, coord, TILE_SIZE, TILE_SIZE);
...
TerrainTile *
gb::TerrainTiler::createTile(GDALDataset *dataset, const TileCoordinate &coord, gb::GDALDatasetReader *reader) const {
  // Copy the raster data into an array
...
GDALTile *
gb::TerrainTiler::createRasterTile(GDALDataset *dataset, const TileCoordinate &coord) const {
  // Ensure we have some data from which to create a tile
  if (dataset && dataset->GetRasterCount() < 1) {
    throw GBException("At least one band must be present in the GDAL dataset");
  }
...
TerrainTiler &
gb::TerrainTiler::operator=(const TerrainTiler &other) {
