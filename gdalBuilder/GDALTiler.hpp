...
public:
  ...
  lowerLeftTile(i_zoom zoom) const {
    return mGrid.crsToTile(bounds_coordinates.getLowerLeft(), zoom);
  }

  /// Get the upper right tile for a particular zoom level
  inline TileCoordinate
  upperRightTile(i_zoom zoom) const {
    return mGrid.crsToTile(bounds_coordinates.getUpperRight(), zoom);
  }

  /// Get the tile bounds for a particular zoom level
  inline TileBounds
  tileBoundsForZoom(i_zoom zoom) const {
    TileCoordinate ll = mGrid.crsToTile(bounds_coordinates.getLowerLeft(), zoom),
      ur = mGrid.crsToTile(bounds_coordinates.getUpperRight(), zoom);
  ...
  bounds() const {
    return const_cast<const CRSBounds &>(bounds_coordinates);
  }
...
protected:
  ...
  CRSBounds bounds_coordinates;
  CRSBounds bounds_latLngAlt;
