...
public:
  ...
  static gb::TileCoordinate neighborCoord(const Grid& grid, const gb::TileCoordinate &coord, int borderIndex, bool& okNeighborCoord) {
    ...
    case 0:
      ...
      return gb::TileCoordinate(coord.zoom, coord.x - 1, coord.y);
    ...
    case 1:
      ...
      return gb::TileCoordinate(coord.zoom, coord.x, coord.y + 1);
    ...
    default:
      throw GBException("Bad Neighbor border index");
    }
  }

  /// Apply the activation state of the border of the specified Neighbor.
  void applyBorderActivationState(const gb::chunk::heightfield &hf, int borderIndex) {
    int level = -1;
  ...
  void generateMesh(gb::chunk::mesh &mesh, int level) {
    int x0 = 0;
    int y0 = 0;
