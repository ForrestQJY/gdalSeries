	...
	mHeights(TILE_CELL_SIZE)
{
	...
	if (count + 1 != TILE_CELL_SIZE) {
		throw GBException("Not enough height data");
	}

	// Get the child flag
	if (fread(&(mChildren), 1, 1, fp) != 1) {
		throw GBException("Could not read child tile byte");
	}
	...
	case 1:
		break;
	default:
		throw GBException("Not contain enough water mask data");
	}
}
...
Terrain::readFile(const char* fileName) {
	...
	if (gzread(terrainFile, inflateBuffer, 1) != 0) {
		gzclose(terrainFile);
		throw GBException("File has too many bytes to be a valid terrain");
	}
	gzclose(terrainFile);
	...
	default:                    // it can't be a terrain file
		throw GBException("File has wrong file size to be a valid terrain");
	}
...
Terrain::writeFile(const char* fileName) const {
	GBZFileOutputStream ostream(fileName);
	writeFile(ostream);
}
 ...
 */
void
Terrain::writeFile(GBOutputStream& ostream) const {

	// Write the height data
	ostream.write((const void*)mHeights.data(), TILE_CELL_SIZE * 2);
	// Write the child flags
	ostream.write(&mChildren, 1);
	// Write the water mask
	ostream.write(&mMask, mMaskLength);
}
...
TerrainTile::heightsToRaster() const {
	...
	if (hDstDS == NULL) {
		CPLFree(pszDstWKT);
		throw GBException("Could not create in memory raster");
	}
	...
	if (GDALSetProjection(hDstDS, pszDstWKT) != CE_None) {
		GDALClose(hDstDS);
		CPLFree(pszDstWKT);
		throw GBException("Could not set projection on in memory raster");
	}
	CPLFree(pszDstWKT);
	...
	if (GDALRasterIO(hBand, GF_Write, 0, 0, tileSize, tileSize,
		...
		GDALClose(hDstDS);
		throw GBException("Could not write heights to in memory raster");
	}
